 ...
 * @author chenlan
 */
public class TestDisposeCloverXml {
	/**
	 * clover报告的工程路径名称
	 */
	 ...
	 * 记录的覆盖率信息
	 */
	private static List<CovereBean> list = new ArrayList<CovereBean>();
	...
	private static Map<String, String> getExclude(String filename) throws Exception {
		...
		InputStream is = TestDisposeCloverXml.class.getResourceAsStream(filename);
		try {
	...
	public void testGetCloverByPath_i_es() throws Exception {
		...
		double es_clover = TestDisposeCloverXml.getCloverByPath(CLOVERXML, "esenface", getExclude("i_es_pacexclude.txt"),
	...
	public void testGetCloverByPath_i() throws Exception {
		...
		double i_clover = TestDisposeCloverXml.getCloverByPath(CLOVERXML, "irpt", getExclude("i_pacexclude.txt"),
	 ...
	 * @throws Exception 
	 */
	private static double getCloverByPath(String xmlpath, String project, Map<String, String> excludes, Map<String, String> exclass) throws Exception {
			...
			for (int i = 0; i < rootlist.getLength(); i++) {
				...
				if (projectnode instanceof Element) {
					if (PROJECT.equals(projectnode.getNodeName())) {
						getclover(projectnode, PACKAGE, FILES, project, excludes, exclass, null);
					}
				}
			}
		} finally {
		...
		for (Iterator<CovereBean> iterator = list.iterator(); iterator.hasNext();) {
			CovereBean cloverbean = iterator.next();
	...
	private static void getclover(Node node, String name, String attribute, String project, Map<String, String> excludes, Map<String, String> exclass, CovereBean coverebean) throws CloneNotSupportedException {
			...
			if (cnode instanceof Element) {
				if (METRICS.equals(cnode.getNodeName())) {
					if (StrFunc.isNull(attribute)) {
						...
						cb.setCoveredelements(StrFunc.str2int(((Element) cnode).getAttribute(COVEREDELEMENTS), 0));
					...
					if (FILE.equals(cnode.getNodeName())) {
						if (((Element) cnode).getAttribute(PATH).startsWith(RESPONSEPATH + project)) {
							...
							if (new File(path).exists()) {
								...
								cb.setName(path);
								getclover(cnode, CLASS, null, project, excludes, exclass, cb);
							}
						} else {
							// 如果不是指定工程下的不统计
							break;
						...
						if (null != excludes && !excludes.isEmpty()) {
							...
							getclover(cnode, FILE, CLASSES, project, excludes, exclass, coverebean);
					}
				}
			}
		}
...
class CovereBean implements Comparable<CovereBean>, Cloneable {
	/**
	 * 覆盖分支数
	 */
	...
	public String toString() {
		return name + "," + coveredelements + "," + elements + "," + (isexclude ? "是" : "否") + ","
				+ StrFunc.null2blank(exccontent) + "," + StrFunc.null2blank(description) + "\r\n";
	}

	public int compareTo(CovereBean o) {
	...
	@Override
	protected CovereBean clone() throws CloneNotSupportedException {
		CovereBean coverebean = new CovereBean();
		coverebean.setCoveredelements(coveredelements);
		coverebean.setDescription(description);
